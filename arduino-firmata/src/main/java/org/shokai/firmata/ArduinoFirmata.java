...
public class ArduinoFirmata{
    ...
    public ArduinoFirmata(android.app.Activity context){
        ...
        if(drivers.size() > 0) {
            ...
            if(ports.size() > 0) {
                m_port = ports.get(0);
                m_connection = manager.openDevice(m_port.getDriver().getDevice());
            }
        }
    }
    ...
    public void connect() throws IOException, InterruptedException{
        ...
        if(m_port == null) throw new IOException("device not found");
        try{
            //this.usb.open();
    ...
    public void write(byte[] writeData){
        try{
            if(this.isOpen()) this.m_port.write(writeData, 100);
        }
        catch(IOException e){
            this.close();
            if(handler!=null) handler.onClose();
